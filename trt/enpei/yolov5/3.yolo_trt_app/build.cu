...
class CalibrationDataReader : public IInt8MinMaxCalibrator {
 ...
 public:
    ...
    CalibrationDataReader(const std::string &dataDir, const std::string &list, int batchSize = 1)
        ...
        std::cout << "CalibrationDataReader: " << mFileNames.size() << " images, " << mBatchCount
                  << " batches." << std::endl;
    }

    int32_t getBatchSize() const noexcept override { return mBatchSize; }
    // 用于提供一批校准数据。在该方法中，需要将当前批次的校准数据读取到内存中，并将其复制到设备内存中。然后，将数据指针传递给
    // TensorRT 引擎，以供后续的校准计算使用。
...
int main(int argc, char **argv) {
    ...
    auto parsed = parser->parseFromFile(onnx_file_path,
