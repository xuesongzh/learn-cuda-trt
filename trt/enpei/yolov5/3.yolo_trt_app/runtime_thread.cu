...
class PersonApp {
 public:
    // destructor
    ~PersonApp() { std::cout << "PersonApp destructor" << std::endl; }
    // constructor
    PersonApp(const std::string &engine_file, const std::string &input_video_path, int mode,
              float dist_threshold, int do_stream, int bitrate)
        ...
        std::cout << "width: " << frameSize_.width << " height: " << frameSize_.height
    ...
    void readFrame() {
        ...
        while (cap.isOpened()) {
            ...
            // 如果缓存满了，就等待
            stage_1_not_full.wait(lock, [] { return stage_1_frame.size() < BUFFER_SIZE; });
            // 增加一个元素
    ...
    void inference() {
        ...
        while (true) {
            ...
            // 使用{} 限制作用域，否则锁会在一次循环结束后才释放
            {
                ...
                std::unique_lock<std::mutex> lock(stage_1_mutex);
                // 如果缓存为空，就等待
                stage_1_not_empty.wait(lock, [] { return !stage_1_frame.empty(); });
                // 取出一个元素
...
#endif
            {
                ...
                // not full
                stage_2_not_full.wait(lock2, [] { return stage_2_buffer.size() < BUFFER_SIZE; });
                //   push
    ...
    void postprocess() {
        ...
        // cv::VideoWriter writer("./output/record.mp4", cv::VideoWriter::fourcc('H', '2', '6',
        // '4'), video_fps_, frameSize_);
        ...
        while (true) {
...

            {
                ...
                // 如果缓存为空，就等待
                stage_2_not_empty.wait(lock, [] { return !stage_2_buffer.empty(); });
                // 取出一个元素
            ...
            for (size_t j = 0; j < item.bboxs.size(); j++) {
                ...
                // 绘制labelid
                // cv::putText(frame, std::to_string((int)bboxs[j].class_id), cv::Point(r.x, r.y -
                // 10), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0x27, 0xC1, 0x36), 2);
            }
            // 获取聚集点
            ...
            for (size_t i = 0; i < gather_points.size(); i++) {
                ...
                for (size_t j = 0; j < gather_points[i].size(); j++) {
                    // std::cout << "聚集点：" << gather_points[i][j].x << "," <<
                    // gather_points[i][j].y << std::endl; 绘制聚集点
                    blender_overlay(gather_points[i][j].x, gather_points[i][j].y, 80, frame, 0.3,
            ...
            auto elapsed_3 =
                std::chrono::duration_cast<std::chrono::microseconds>(end_3 - start_3).count() /
                1000.f;
            ...
            cv::putText(frame, time_str, cv::Point(50, 50), cv::FONT_HERSHEY_PLAIN, 3,
                        cv::Scalar(255, 255, 255), 2);
            cv::putText(frame, fps_str, cv::Point(50, 100), cv::FONT_HERSHEY_PLAIN, 3,
                        cv::Scalar(255, 255, 255), 2);
...

            {
                ...
                // 如果缓存满了，就等待
                stage_3_not_full.wait(lock2, [] { return stage_3_frame.size() < BUFFER_SIZE; });
                // 添加一个元素
    ...
    void streamer() {
        ...
        streamer::StreamerConfig streamer_config(frameSize_.width, frameSize_.height,
                                                 frameSize_.width, frameSize_.height, video_fps_,
                                                 bitrate, "main", "rtmp://localhost/live/mystream");
        ...
        while (true) {
            ...
            }
            {
                ...
                // 如果缓存为空，就等待
                stage_3_not_empty.wait(lock, [] { return !stage_3_frame.empty(); });
                // 取出一个元素
            ...
            auto elapsed_4 =
                std::chrono::duration_cast<std::chrono::microseconds>(end_4 - start_4).count() /
                1000.f;
 ...
 private:
    ...
    // 混合图像
    void blender_overlay(int x, int y, int radius, cv::Mat &image, float alpha, int height,
                         int width) {
        // initial
        int rect_l = x - radius;
        int rect_t = y - radius;
...
int main(int argc, char **argv) {
    if (argc < 7) {
        std::cerr << "用法: " << argv[0]
                  << " <engine_file> <input_path_path> <preprocess_mode> <dist_threshold> <stream> "
                     "<bitrate> "
                  << std::endl;
        return -1;
    }
