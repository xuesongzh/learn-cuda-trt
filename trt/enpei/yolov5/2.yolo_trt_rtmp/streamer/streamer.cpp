...
static int encode_and_write_frame(AVCodecContext *codec_ctx, AVFormatContext *fmt_ctx,
                                  AVFrame *frame) {
    AVPacket pkt = {0};
    av_init_packet(&pkt);
...
static int set_options_and_open_encoder(AVFormatContext *fctx, AVStream *stream,
    ...
    stream->time_base =
        codec_ctx->time_base;  // will be set afterwards by avformat_write_header to 1/1000
...
void Streamer::stream_frame(const uint8_t *data) {
    if (can_stream()) {
        ...
        sws_scale(scaler.ctx, &data, stride, 0, config.src_height, picture.frame->data,
...
void Streamer::enable_av_debug_log() { av_log_set_level(AV_LOG_DEBUG); }
...
int Streamer::init(const StreamerConfig &streamer_config) {
    ...
    if (!(format_ctx->oformat->flags & AVFMT_NOFILE)) {
        int avopen_ret =
            avio_open2(&format_ctx->pb, config.server.c_str(), AVIO_FLAG_WRITE, nullptr, nullptr);
        if (avopen_ret < 0) {
    ...
    if (set_options_and_open_encoder(format_ctx, out_stream, out_codec_ctx, out_codec,
                                     config.profile, config.dst_width, config.dst_height,
    ...
    scaler.init(out_codec_ctx, config.src_width, config.src_height, config.dst_width,
                config.dst_height, SWS_BILINEAR);
