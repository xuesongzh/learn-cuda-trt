...
class NNEngine {
 public:
    ...
    virtual nn_error_e LoadModelFile(
        const char *model_file) = 0;  // 加载模型文件，=0表示纯虚函数，必须在子类中实现
    ...
    virtual nn_error_e Run(std::vector<tensor_data_s> &inputs, std::vector<tensor_data_s> &outpus,
                           bool want_float) = 0;  // 运行模型
};
