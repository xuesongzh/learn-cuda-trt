...
void DetectionGrp2DetectionArray(yolov5::detect_result_group_t &det_grp,
    ...
    for (int i = 0; i < det_grp.count; i++) {
        ...
        det.box = cv::Rect(det_grp.results[i].box.left, det_grp.results[i].box.top,
...
nn_error_e Yolov5::Postprocess(const cv::Mat &img, std::vector<Detection> &objects) {
    ...
    float scale_w = height / img.cols;
    float scale_h = width / img.rows;

    yolov5::detect_result_group_t detections;

    yolov5::post_process((int8_t *)output_tensors_[0].data, (int8_t *)output_tensors_[1].data,
                         (int8_t *)output_tensors_[2].data, height, width, BOX_THRESH, NMS_THRESH,
                         scale_w, scale_h, out_zps_, out_scales_, &detections);
